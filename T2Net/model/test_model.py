...
class TestModel(BaseModel):
    ...
    def set_synthetic(self, input):
        ...
        self.image_paths = input['img_source_paths']
    ...
    def test_syn2real2task(self):
        self.img_s = Variable(self.img_source)

        with torch.no_grad():
            self.img_s2t = self.net_s2t.forward(self.img_s)
            self.lab_t_g = self.net_img2task.forward(self.img_s2t[-1])
    ...
    def save_real(self, visualizer, wed_page):
        img_target_paths = self.input['img_target_paths']
    ...
    def save_synthetic(self, visualizer, wed_page):
        img_source_paths = self.input['img_source_paths']
        ...
        for i in range(self.img_s.size(0)):
            ...
            visuals = OrderedDict([('img_s', img_source), ('img_s2t', img_source2target), ('lab_t_g', lab_fake_target)])
            print('process image ......%s' % img_source_paths[0])
            visualizer.save_images(wed_page, visuals, img_source_paths)
            img_source_paths.pop(0)
