...
def load_dataset(directory):
    ...
    for root, dirs, files in walk(directory):
        for fi in files:
            filelist.append(root + '/' + fi)
    return {name: open(name,'rb').read() for name in filelist}

#================
# Iris dataset
#================

def iris_resize(img, resolution=256, padding=6):
    from skimage.transform import resize
    return resize(img, (resolution, resolution), preserve_range=True, mode='reflect', anti_aliasing=True ) #return resize(img, (resolution, int(resolution*4/3)), preserve_range=True, mode='reflect', anti_aliasing=True )


def get_iris_data(batch_size, iris_data_directory='iris'):
    # Train in the micro dataset:
    data_train = open('data/micro_S2R.txt','rb').read()
    data_test  = open('data/micro_S2R.txt','rb').read()
    ...
    # Helpful for testing...
    if False:
        iris_train = iris_train[:250]
        iris_test = iris_test[:25]

    return iris_train, iris_test, shape_rgb, shape_depth


def get_iris_train_test_data(batch_size):
    iris_train, iris_test, shape_rgb, shape_depth = get_iris_data(batch_size)

    train_generator = Iris_BasicRGBSequence(iris_train, batch_size=batch_size, shape_rgb=shape_rgb, shape_depth=shape_depth)
    test_generator  = Iris_BasicRGBSequence(iris_test , batch_size=batch_size, shape_rgb=shape_rgb, shape_depth=shape_depth)

    return train_generator, test_generator


class Iris_BasicRGBSequence(Sequence):
